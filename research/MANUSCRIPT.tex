...
On real data --- a CIMMYT wheat panel ($n=599$), a PIC pig ($n=3534$, 52k SNP) and a heterogeneous-stock mouse panel ($n=1814$, with real sex) --- support-first reaches the exact optimum, agreeing with the conic optimum to $10^{-8}$; at matched realised coancestry it agrees with the interior-point methods, and where they halt just inside the constraint support-first reaches the boundary, so its small edge is the diversity budget they leave unspent rather than a different optimum --- the shipped matrix-free solver, forming no $\bG$, running 18$\times$--182$\times$ faster end to end (and, handed $\bG$, the active set alone up to ${\sim}2500\times$ faster), and ${\sim}76000\times$ faster than a general conic interior-point solver at $n=10000$. Against AlphaMate, a heuristic for the distinct problem of discrete mate allocation, the exact optimum is no worse at matched coancestry on the continuous relaxation the two share, at a small fraction of the run time. Across synthetic populations the optimal support stays 14--19 as $n$ grows from 1000 to 40000, while the dense $\bG$ the alternatives must form reaches 11.9\,GiB --- a 40$\times$ larger footprint than $\bZ$, past the working memory of an 8--16\,GB laptop --- in a regime where support-first still solves in under 0.1\,s. Support-first makes exact, reproducible optimum contribution selection practical at genomic scale on a laptop.
...
\noindent\textsuperscript{1}\,active set given a precomputed dense $\bG$, solve only --- the regime optiSel runs in (NumPy prototype); \textsuperscript{2}\,matrix-free, forming no $\bG$, end to end including the R-binding genotype copy (shipped Rust); \textsuperscript{3}\,optiSel\,/\,matrix-free.

The speed-up column is the shipped solver, forming nothing, against optiSel: 18--182$\times$. The algorithm column --- the active set handed $\bG$ as the interior-point tools are --- reaches the same optimum ${\sim}90$--2500$\times$ faster (the pig's ${\sim}2500\times$ the largest, and the origin of this work's earlier figure), but pays the $O(n^2 m)$ to build and $O(n^2)$ to store $\bG$ that the matrix-free solver never does; handed $\bG$ the solve avoids streaming $\bZ$, so it is the faster of the two on every panel, and the matrix-free column is what that costs to keep nothing dense in memory. On the pig much of the shipped 0.622\,s is the R binding copying the 1.5\,GB genotype matrix, so at $m=52843$ data movement, not the solve, sets the end-to-end number. Both columns return the same optimum on every row, reaching the boundary optiSel stops just inside. The mouse row is the true sexed OCS, on a genuine recorded sex (934 males, 880 females); sex is real only for the mouse panel, and the selection criterion is a recorded phenotype or EBV used as a proxy for a genomic breeding value.
