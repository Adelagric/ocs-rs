...
\begin{table}[ht]
\centering
\caption{support-first vs optiSel, same optimum throughout, one idle R session on one instance (Apple M4 Max); each time a median (matrix-free and algorithm 5$\times$, optiSel 3$\times$). Two support-first columns: the active set given a dense $\bG$ (the regime optiSel runs in), and the shipped matrix-free solver, which forms no $\bG$, timed end to end from genotypes including the R-binding copy.}
\label{tab:optisel}
