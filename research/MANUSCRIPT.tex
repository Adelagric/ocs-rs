...
On real data --- a CIMMYT wheat panel ($n=599$), a PIC pig ($n=3534$, 52k SNP) and a heterogeneous-stock mouse panel ($n=1814$, with real sex) --- support-first reaches the exact optimum, agreeing with the conic optimum to $10^{-8}$; at matched realised coancestry it agrees with the interior-point methods, and where they halt just inside the constraint support-first reaches the boundary, so its small edge is the diversity budget they leave unspent rather than a different optimum --- the shipped matrix-free solver, forming no $\bG$, running 18$\times$--193$\times$ faster end to end (and, handed $\bG$, the active set alone up to 2445$\times$ faster), and ${\sim}37000\times$ faster than a general conic interior-point solver at $n=10000$. Against AlphaMate, a heuristic for the distinct problem of discrete mate allocation, the exact optimum is no worse at matched coancestry on the continuous relaxation the two share, at a small fraction of the run time. Across synthetic populations the optimal support stays 14--19 as $n$ grows from 1000 to 40000, while the dense $\bG$ the alternatives must form reaches 11.9\,GiB --- a 40$\times$ larger footprint than $\bZ$, past the working memory of an 8--16\,GB laptop --- in a regime where support-first still solves in under 0.1\,s. Support-first makes exact, reproducible optimum contribution selection practical at genomic scale on a laptop.
...
A generic solver is a soft target; the informative comparison is against optiSel, the standard exact OCS tool, on its own formulation. After extending support-first to the two sex-equality constraints, it returns the same optimum as optiSel. The shipped matrix-free solver --- forming no $\bG$ at all --- runs 18--193$\times$ faster across real and synthetic panels (Table~\ref{tab:optisel}), the factor widening with $n$ as optiSel's interior-point cost climbs. Given $\bG$ precomputed, as the interior-point tools require, the active set alone reaches the same optimum 58--2445$\times$ faster: the algorithmic advantage this section dissects, of which the matrix-free solver keeps most while never building the matrix.

\begin{table}[ht]
\centering
\caption{support-first vs optiSel, same optimum throughout, one R session on one instance (Apple M4 Max). Two support-first columns: the active set given a dense $\bG$ (the regime optiSel runs in), and the shipped matrix-free solver, which forms no $\bG$, timed end to end from genotypes including the R-binding copy.}
\label{tab:optisel}
...
\midrule
synthetic (structured)        & 1000 & 500   & 0.025\,s & 0.120\,s & 2.11\,s  & 18$\times$ \\
synthetic (structured)        & 2000 & 500   & 0.105\,s & 0.282\,s & 13.56\,s & 48$\times$ \\
synthetic (structured)        & 5000 & 500   & 0.504\,s & 0.901\,s & 173.8\,s & 193$\times$ \\
CIMMYT wheat (real GRM)       & 599  & 1279  & 0.011\,s & 0.009\,s & 0.634\,s & 70$\times$ \\
PIC pig (real GRM, 52k SNP)   & 3534 & 52843 & 0.022\,s & 0.611\,s & 53.8\,s  & 88$\times$ \\
HS mouse (real GRM, real sex) & 1814 & 10346 & 0.009\,s & 0.057\,s & 6.94\,s  & \textbf{122$\times$} \\
\bottomrule
\end{tabular}
\end{table}
...
The speed-up column is the shipped solver, forming nothing, against optiSel: 18--193$\times$. The algorithm column --- the active set handed $\bG$ as the interior-point tools are --- reaches the same optimum 58--2445$\times$ faster (the pig's 2445$\times$ the largest, and the origin of this work's earlier figure), but pays the $O(n^2 m)$ to build and $O(n^2)$ to store $\bG$ that the matrix-free solver never does. On the pig the shipped 0.611\,s is mostly the R binding copying the 1.5\,GB genotype matrix --- the solve is ${\sim}0.34$\,s --- so at $m=52843$ that copy, not the solve, sets the end-to-end number; wheat shows the reverse, matrix-free (0.009\,s) beating the dense-$\bG$ prototype (0.011\,s) because at small $m$ the products are cheaper than indexing a dense $\bG$. Both columns return the same optimum on every row, reaching the boundary optiSel stops just inside. The mouse row is the true sexed OCS, on a genuine recorded sex (934 males, 880 females); sex is real only for the mouse panel, and the selection criterion is a recorded phenotype or EBV used as a proxy for a genomic breeding value.
