...
\midrule
synthetic (structured)        & 1000 & 500   & 0.023\,s & 0.116\,s & 2.068\,s & 18$\times$ \\
synthetic (structured)        & 2000 & 500   & 0.102\,s & 0.278\,s & 13.40\,s & 48$\times$ \\
synthetic (structured)        & 5000 & 500   & 0.506\,s & 0.905\,s & 163.0\,s & 180$\times$ \\
CIMMYT wheat (real GRM)       & 599  & 1279  & 0.003\,s & 0.007\,s & 0.596\,s & 85$\times$ \\
PIC pig (real GRM, 52k SNP)   & 3534 & 52843 & 0.020\,s & 0.593\,s & 49.9\,s  & 84$\times$ \\
HS mouse (real GRM, real sex) & 1814 & 10346 & 0.008\,s & 0.054\,s & 6.92\,s  & \textbf{128$\times$} \\
PIC pig, sexed sub-panel\textsuperscript{4} & 1194 & 52843 & 0.004\,s & 0.219\,s & 2.62\,s & 12$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
The speed-up column is the shipped solver, forming nothing, against optiSel: 12--180$\times$. The algorithm column --- the active set handed $\bG$ as the interior-point tools are --- reaches the same optimum ${\sim}90$--2500$\times$ faster (the pig's ${\sim}2500\times$ the largest, and the origin of this work's earlier figure), but pays the $O(n^2 m)$ to build and $O(n^2)$ to store $\bG$ that the matrix-free solver never does; handed $\bG$ the solve avoids streaming $\bZ$, so it is the faster of the two on every panel, and the matrix-free column is what that costs to keep nothing dense in memory. On the pig the binding no longer copies the 1.5\,GB genotype matrix at all --- R and the solver's linear algebra are both column-major, so the buffer is viewed in place --- which leaves the solve itself, and the marker count it streams, setting the end-to-end number. Both columns return the same optimum on every row, reaching the boundary optiSel stops just inside.
