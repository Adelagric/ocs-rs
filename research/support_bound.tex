...
The universal question is closed, negatively, by Theorem~\ref{thm:noridge}; the useful one is
\emph{conditional}: under structural assumptions that real genomic relationship matrices satisfy
--- a spectrum that decays with no large degenerate floor, and a breeding-value vector not
concentrated on the top eigendirections --- prove $|S| \le$ (something small, $n$-independent).
A natural route couples the spectral gap with $\bb$'s projection onto the top eigenspace; the
growth half ($|S|$ increasing as $k \to 0$) is governed in turn by the classical
contributions\,$\leftrightarrow\,\Delta F\,\leftrightarrow\,N_e$ link (Wray \& Thompson 1990;
Woolliams \& Bijma 2000). This is the meeting point of optimisation (the face dimension of the
perturbed second-order cone) and quantitative genetics (the effective number of contributing
lineages).
