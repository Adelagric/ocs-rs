...
if have cargo; then
  ...
  # The manuscript's Table 1 sits at the operating point (support ~100, dF ~1%):
  # m=20000 and k = 0.017 x mean diag G. The default --k-frac 0.6 is the loose cap
  # quoted only as the "~76000x" extreme (research/repro/table1_numbers.md).
  for n in $ns; do
    cargo run --release --quiet -- compare --n "$n" --m 20000 --k-frac 0.017 || echo "  compare n=$n failed"
  done
else
  echo "SKIP: cargo not found"
fi

banner "3b/7 Table 1 baseline check — Clarabel solver generated by a third party (cvxgenrust)"
if have uv && have cargo; then
  # research/repro/cvxgenrust/RESULTS.md. Generation is O(n^2) in memory (8 GB at
  # n=1000), so the check stops at n=1000.
  bash research/repro/cvxgenrust/run.sh 500 1000 || echo "  cvxgenrust check failed"
else
  echo "SKIP: uv not found (needed to install cvxgenrust into a local venv)"
fi
