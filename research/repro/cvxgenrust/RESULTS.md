...
timed by `dump_ocs`, the generated crate by `ocs_bench.rs.tmpl`.
...
- **Explicit zeros.** The generator sees `L` as an opaque `n×n` parameter, so its
  `A` carries the strict upper triangle of `Lᵀ` as `n(n−1)/2` explicit zeros
  (124 750 at n=500, 499 500 at n=1000; `A` nnz = n²+2n+2). Left in, the generated
  solver is 4.9× (n=500) / 3.1× (n=1000) slower; with Clarabel's
  `input_sparse_dropzeros = true` it is at parity. The crate's `socp.rs` emits
  only the triangle, which is the same thing done by hand.
...
| n | `generate_code` | peak RSS | generated `src/` | `cargo build --release` | binary |
|---|---|---|---|---|---|
| 500 | 0.5 s | 2.3 GB | 10.5 MB | 12 s | — |
| 1000 | 16.7 s | 8.1 GB | 43.5 MB | 19 s (1.9 GB) | 33 MB |

Peak memory and source size grow as n² (CVXPY's DPP tensor holds the n² parameter
entries; the affine maps are emitted as static literals in `data.rs`): ~32 GB at
n=2000, ~200 GB at n=5000. Not attempted above n=1000 on this machine. This is
...
- Established: the Table-1 baseline is a correctly assembled Clarabel cone
  ...
  cost. The active-set speed-ups of Table 1 are not an artefact of the baseline.
