...
- **3.2 Speed & scaling.** vs Clarabel to n=10000 (76179×: 1792 s → 0.024 s, Rust vs Rust, `research/repro/table1_numbers.md`); vs optiSel: shipped matrix-free 12×–180× end to end, algorithm given G ~90×–2500× (`research/repro/table2_numbers.md`).
...
| ~76000× vs Clarabel at n=10000 | **Measured** | idle M4 Max, one serial sweep, Rust vs Rust in one process (`compare --n N`); 1792 s → 0.024 s; supersedes the earlier 37090× (pre-optimisation, no stated provenance) |
| vs optiSel: shipped 12×–180×, algorithm ~90×–2500× | **Measured** | M4 Max, idle, medians (matrix-free/algorithm 5×, optiSel 3×), all seven panels; pig shipped 0.593 s (84×) / algorithm 0.020 s (~2500×) vs optiSel 49.9 s; two-column Table 2, `research/repro/table2_numbers.md` |
