...
Un solveur générique est une cible facile ; la comparaison informative se fait contre optiSel, l'outil OCS exact de référence, sur sa propre formulation. Après extension de support-first aux deux contraintes d'égalité de sexe, il renvoie le même optimum qu'optiSel. Le solveur sans matrice livré — qui ne forme aucun **G** — s'exécute 18× à 182× plus vite sur des panels réels et synthétiques (Tableau 2), le facteur s'élargissant avec n à mesure que le coût du point intérieur d'optiSel augmente. Avec **G** précalculée, comme l'exigent les outils à point intérieur, l'ensemble actif seul atteint le même optimum ~90× à 2500× plus vite : l'avantage algorithmique que cette section dissèque, dont le solveur sans matrice échange une partie pour ne jamais construire la matrice.

**Tableau 2.** support-first contre optiSel, même optimum partout, une seule session R idle sur une même instance (Apple M4 Max) ; chaque temps est une médiane (sans matrice et algorithme 5×, optiSel 3×). Deux colonnes support-first : l'ensemble actif avec **G** dense fournie (le régime d'optiSel), et le solveur sans matrice livré, qui ne forme aucun **G**, chronométré de bout en bout depuis les génotypes, copie du *binding* R comprise.
