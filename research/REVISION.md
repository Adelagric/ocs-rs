...
## Post-0.3.0 audit (2026-09-20): independent baseline, a regression, a retracted number

Triggered by checking the Table-1 conic baseline against a third-party code
generator (`cvxgenrust`, Zhu & Boedecker 2026 — provenance in
`research/repro/cvxgenrust/RESULTS.md`). Three outcomes, in order of weight.

**1. Regression in the shipped 0.3.0 (fixed).** The `Kinship` refactor of
`e10e2f0` dropped `GramCache`'s contiguous copy of the support rows of `Z` and read
each Gram entry through `MatRef` indexing instead — a stride-`n` walk through the
column-major `Z` that misses the cache on every marker. Back to back on the same
instance (`compare --n 1000 --m 20000 --k-frac 0.017`): support-first **7.52 s at
0.3.0 vs 1.64 s at `bb8d90e`** (4.6×), identical optimum and product count (151).
Fix: `Kinship::dense_z()` lets the cache gather rows once for the dense
representation (the packed columns are already contiguous and keep the entry-wise
path); 1.64–1.65 s after the fix, bit-identical Gram (summation order unchanged),
43 tests, clippy `-D warnings`, fmt. **Table 1 is unaffected**: its support-first
column was measured on 2026-07-28, before the refactor (2026-07-29 17:00), and is
reproduced by the fixed tree (1.43 s idle then; 1.65 s under today's load). Table 2's
matrix-free column likewise predates it (R binding built 2026-07-28). **The PyPI wheel
and the git tag `v0.3.0` carry the regression** → 0.3.1.

**2. "3.1× faster with 2-bit packing" retracted.** `examples/packed_bench.rs` was
run on 2026-07-29 *after* the refactor, so its dense side was the regressed path
(14.6 s); packed 4.71 s ⇒ the 3.1× in the Discussion. With the dense path fixed the
same benchmark gives dense 3.41–3.48 s and packed 6.39–6.66 s (3 runs, **loaded
machine**, load average 12–14; the packed product is threaded and suffers most from
contention). Idle packed (July, 4.71 s) against today's fixed dense (3.4 s) is 1.4×
slower; today's pair is 1.9× slower. Manuscript (EN+FR, Discussion) now reads: 32×
...
**3. Baseline independently reproduced.** A solver generated by `cvxgenrust` from a
six-line CVXPY model of Route A returns the same optimum (gain to 2e-8) and the same
per-iteration cost (within 3 % of a binary of this tree; 16 % *slower* than the binary
Table 1 was measured with) at n=500 and n=1000, provided the factor is declared with its
triangular sparsity pattern (`cp.Parameter(..., sparsity=np.tril_indices(n))`): the
generator then carries only the triangle and is at parity out of the box (n=1000:
1.98 s, 19 it). Declared dense, it carries n(n−1)/2 explicit zeros and is 3–5× slower
unless Clarabel drops them — a modelling choice on the user's side, not a defect.
The generator needs 8 GB and 43 MB of source at n=1000 and grows as n², so the
check stops there. Added to the manuscript (Methods baselines, §Clarabel, references:
Mattingley & Boyd 2012, Schaller et al. 2022, Zhu & Boedecker 2026; DOIs verified).
Side finding: the *same* Clarabel build runs 14 % slower per iteration when `ocs_rs`
is compiled with `src/packed.rs` present (never executed, not linked into the test
binary) — a layout effect, bisected, not understood further; it bounds the n=1000
ratio of Table 1 and nothing else. Table 1 used the faster (conservative) binary.
