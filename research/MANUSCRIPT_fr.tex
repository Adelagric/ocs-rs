...
Un solveur générique est une cible facile ; la comparaison informative se fait contre optiSel, l'outil OCS exact de référence, sur sa propre formulation. Après extension de support-first aux deux contraintes d'égalité de sexe, il renvoie le même optimum qu'optiSel. Le solveur sans matrice livré --- qui ne forme aucun $\bG$ --- s'exécute 18$\times$ à 193$\times$ plus vite sur des panels réels et synthétiques (Tableau~\ref{tab:optisel}), le facteur s'élargissant avec $n$ à mesure que le coût du point intérieur d'optiSel augmente. Avec $\bG$ précalculée, comme l'exigent les outils à point intérieur, l'ensemble actif seul atteint le même optimum 58$\times$ à 2445$\times$ plus vite : l'avantage algorithmique que cette section dissèque, dont le solveur sans matrice conserve l'essentiel sans jamais construire la matrice.

\begin{table}[ht]
\centering
\caption{support-first contre optiSel, même optimum partout, une seule session R sur une même instance (Apple M4 Max). Deux colonnes support-first : l'ensemble actif avec $\bG$ dense fournie (le régime d'optiSel), et le solveur sans matrice livré, qui ne forme aucun $\bG$, chronométré de bout en bout depuis les génotypes, copie du \emph{binding} R comprise.}
\label{tab:optisel}
...
\midrule
synthétique (structuré)        & 1000 & 500   & 0,025~s & 0,120~s & 2,11~s  & 18$\times$ \\
synthétique (structuré)        & 2000 & 500   & 0,105~s & 0,282~s & 13,56~s & 48$\times$ \\
synthétique (structuré)        & 5000 & 500   & 0,504~s & 0,901~s & 173,8~s & 193$\times$ \\
blé CIMMYT (GRM réelle)        & 599  & 1279  & 0,011~s & 0,009~s & 0,634~s & 70$\times$ \\
porc PIC (GRM réelle, 52k SNP) & 3534 & 52843 & 0,022~s & 0,611~s & 53,8~s  & 88$\times$ \\
souris HS (GRM réelle, sexe réel) & 1814 & 10346 & 0,009~s & 0,057~s & 6,94~s & \textbf{122$\times$} \\
\bottomrule
\end{tabular}
\end{table}
...
La colonne accélération est celle du solveur livré, qui ne forme rien, face à optiSel : 18$\times$ à 193$\times$. La colonne algorithme --- l'ensemble actif recevant $\bG$ comme les outils à point intérieur --- atteint le même optimum 58$\times$ à 2445$\times$ plus vite (le 2445$\times$ du porc le plus grand, et l'origine du chiffre antérieur de ce travail), mais paie le $O(n^2 m)$ de construction et le $O(n^2)$ de stockage de $\bG$ que le solveur sans matrice évite. Sur le porc, les 0,611~s livrés sont surtout la copie par le \emph{binding} R de la matrice de génotypes de 1,5~Go --- le solve fait ${\sim}0{,}34$~s --- de sorte qu'à $m=52843$ c'est cette copie, non le solve, qui fixe le temps bout en bout ; le blé montre l'inverse, le sans-matrice (0,009~s) battant le prototype à $\bG$ dense (0,011~s) car à petit $m$ les produits coûtent moins que l'indexation d'un $\bG$ dense. Les deux colonnes renvoient le même optimum sur chaque ligne, atteignant la frontière où optiSel s'arrête juste à l'intérieur. La ligne souris est la véritable OCS sexuée, sur un sexe réellement enregistré (934 mâles, 880 femelles) ; le sexe n'est réel que pour le panel souris, et le critère de sélection est un phénotype ou une VE enregistrée servant de substitut à une vraie valeur génétique génomique.
