...
Un solveur générique est une cible facile ; la comparaison informative se fait contre optiSel, l'outil OCS exact de référence, sur sa propre formulation. Après extension de support-first aux deux contraintes d'égalité de sexe, il renvoie le même optimum qu'optiSel. Le solveur sans matrice livré --- qui ne forme aucun $\bG$ --- s'exécute 18$\times$ à 182$\times$ plus vite sur des panels réels et synthétiques (Tableau~\ref{tab:optisel}), le facteur s'élargissant avec $n$ à mesure que le coût du point intérieur d'optiSel augmente. Avec $\bG$ précalculée, comme l'exigent les outils à point intérieur, l'ensemble actif seul atteint le même optimum ${\sim}90$ à 2500$\times$ plus vite : l'avantage algorithmique que cette section dissèque, dont le solveur sans matrice échange une partie pour ne jamais construire la matrice.
