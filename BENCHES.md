...
### 2026-09-20 — regression found and fixed; packing re-measured; baseline cross-checked

Apple M4 Max, rustc 1.98.1, release (`opt-level 3`, thin LTO). **Not idle**: unrelated
jobs held the load average at 12–19 all session, so absolute times are inflated
(Clarabel by ~3 %, the bandwidth-bound support-first by ~15 %, the threaded packed
product by more); every comparison below is same-session and back to back.

- **Regression.** `compare --n 1000 --m 20000 --k-frac 0.017` (Table 1's row):
  support-first **7.52 s on `v0.3.0` vs 1.64 s on `bb8d90e`**, same 151 products,
  same gain. Cause: the `Kinship` refactor built `G_S` by striding through the
  column-major `Z` (DECISIONS.md). After the fix: 1.64–1.65 s (two builds, four runs),
  Gram bit-identical. n=2000: 2.14 s, support 112, Clarabel 18.3 s.
- **Packed vs dense, pig shape** (`packed_bench`, n=1194, m=52843, k=0.02, 3 runs):
  dense f64 **3.41 / 3.45 / 3.48 s**, packed **6.65 / 6.39 / 6.66 s** — packed is
  **0.51–0.54× the dense speed** here, at 31.9× less memory, identical optimum. The
  July "3.11× faster" was measured against the regressed dense path (14.6 s) and is
  withdrawn. Idle July packed (4.71 s) vs today's fixed dense (3.4 s) is still 0.7×.
  Needs an idle re-run before a single number is quoted; the packed kernel is a
  scalar unpacking loop (~1.6 G genotypes/s on 14 threads idle) with headroom.
- **Baseline cross-check** (`research/repro/cvxgenrust/RESULTS.md`): a Clarabel
  solver generated by a third-party code generator from a CVXPY model of Route A
  matches the crate's conic path in optimum (2e-8) and per-iteration cost (within
  3 % of a binary of this tree) at n=500 and n=1000, once its n(n−1)/2 explicit zeros
